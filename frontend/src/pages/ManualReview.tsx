...
import { FileCode2, Send, AlertTriangle, Bot } from 'lucide-react';
...
export default function ManualReview() {
    ...
    return (
        <div className="flex flex-col gap-6 animate-fade-in">
            <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg flex gap-4 items-start">
                <div className="flex-1">
                    <label className="flex items-center gap-2 text-sm text-gray-400 mb-2 font-medium">
                        <FileCode2 size={16} /> Dán mã nguồn trực tiếp (Source Code)
                    </label>
                    <textarea
                        value={rawCode}
                        onChange={(e) => setRawCode(e.target.value)}
                        rows={8}
                        className="w-full bg-gray-950 border border-gray-600 rounded-lg p-3 text-white focus:outline-none focus:border-blue-500 font-mono text-sm leading-relaxed"
                        placeholder="Dán đoạn code bạn cần bắt lỗi hoặc tối ưu vào đây..."
                    />
                </div>
                ...
                <div className="flex flex-col gap-4 mt-7">
                    ...
                    <button
                        ...
                        className="flex justify-center items-center gap-2 px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-bold transition-all disabled:bg-gray-700 disabled:text-gray-500 shadow-lg shadow-emerald-900/50"
                    >
                        <Send size={18} /> {isLoading ? 'Đang xử lý...' : 'Review Snippet'}
                    </button>
                </div>
            </div>
            ...
            <div className="bg-gray-800 p-8 rounded-xl border border-gray-700 min-h-[400px] shadow-lg">
                {!reviewText && !isLoading && !error && (
                    <div className="text-gray-500 flex flex-col justify-center items-center h-full gap-3 mt-20">
                        ...
                        <p>Chế độ tùy chọn: Dán một đoạn code bất kỳ, AI sẽ phân tích Clean Code & SOLID cho bạn.</p>
                    </div>
                )}
